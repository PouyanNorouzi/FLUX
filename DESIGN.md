...
## 14. Live Tailwind Tokens

The live Tailwind v4 theme is defined in [src/routes/layout.css](/home/pouyan/Projects/recepies-app/src/routes/layout.css).

Implemented token categories:

- Colors: semantic FLUX palette plus ergonomic aliases (`primary`, `surface`, `muted`, `background-light`, `background-dark`).
- Font families: `display` maps to Space Grotesk and `mono` maps to IBM Plex Mono.
- Border widths: `hairline`, `secondary`, and `heavy` map to 1px, 2px, and 4px.
- Box shadows: `hard`, `hard-hover`, and `hard-error` match the Stitch export offsets exactly.
- Radius: all Tailwind radius scale tokens are set to `0px` to preserve hard geometry.

This makes the code implementation and the design spec share the same source of truth for core visual tokens.
